...
function writeToGradleProperties(text: string, next: string): string {
    ...
    if (!changed) throw new Error("gradle.properties에 version=... 라인이 없어 수정 불가");
    return out.join("\n");
}

function readFromBuildGradle(text: string, fileName: string): { before: string } {
...
function writeToBuildGradle(text: string, fileName: string, next: string): string {
    const isKts = fileName.endsWith(".kts");
    const re = isKts
        ? /^(\s*version\s*=\s*")([^"]+)(".*)$/m
        : /^(\s*version\s*=\s*['"])([^'"]+)(['"].*)$/m;
