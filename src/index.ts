...
function assertAllowedAssociation(allowedCsv: string) {
    const allowed = new Set(allowedCsv.split(",").map(s => s.trim().toUpperCase()).filter(Boolean));
...
async function pickUpdater(tool: Tool, prefer: Prefer) {
    ...
    throw new Error("지원되는 빌드 파일을 찾지 못함: pom.xml 또는 gradle.properties/build.gradle(.kts)");
}
...
async function run() {
    try {
        ...
        const defaultBump = (core.getInput("defaultBump") || "patch") as "patch" | "minor" | "major";
        ...
        const commitMessageTemplate = core.getInput("commitMessageTemplate") || "chore(version): {tool} {before} -> {after}";
        const authorName = core.getInput("authorName") || "version-bot";
        const authorEmail = core.getInput("authorEmail") || "version-bot@users.noreply.github.com";

        const parsed: ParsedLine | null = parseCommandLine(comment, commandPrefix, defaultBump);
        if (!parsed) {
        ...
        const tool = parsed.options.tool ? normalizeTool(parsed.options.tool) : baseTool;
        const prefer = parsed.options.prefer ? normalizePrefer(parsed.options.prefer) : basePrefer;
        const keepSnapshot = parsed.options.keepSnapshot != null ? parsed.options.keepSnapshot : baseKeepSnapshot;
        const dryRun = parsed.options.dryRun != null ? parsed.options.dryRun : baseDryRun;

        const target = (parsed.options.target ?? "").trim();

        if (target) {
            const p = path.resolve(process.cwd(), target);
            if (!(await fileExists(p))) throw new Error(`--target 파일 없음: ${target}`);
        }
        ...
        if (dryRun) {
            core.info(`[dryRun] tool=${toolId}, file=${targetPath}, ${before} -> ${after}`);
            return;
        }
        ...
        core.info(`완료: tool=${toolId}, file=${targetPath}, ${before} -> ${after}`);
    } catch (e: any) {
