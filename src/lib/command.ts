...
export const parseCommandLine = (
  ...
  if (!rest) {
    throw new Error(
      `Invalid command format. e.g. ${prefix} bump patch | ${prefix} set 1.2.3`,
    );
  }
  ...
  if (verb === "bump") {
    ...
    if (!version)
      throw new Error(
        `Missing version for set command. e.g. ${prefix} set 1.2.3`,
      );
...
const ensureBumpStrategy = (v: string): BumpStrategy => {
  const s = String(v || "")
    .trim()
    .toLowerCase();
